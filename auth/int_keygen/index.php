...
$PAGE->set_url("$CFG->httpswwwroot/auth/int_keygen/index.php");
...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
	...
	$frm->username = trim(textlib::strtoupper($frm->username));
    ...
    /// Let's get them all set up.
        add_to_log(SITEID, 'user', 'login_keygen', "view.php?id=$USER->id&course=".SITEID,
                   $user->id, 0, $user->id);
    ...
    /// Currently supported only for ldap-authentication module
        ...
        redirect(new moodle_url($authplugin->get_login_url(), array('testsession'=>$USER->id)));

    } else {
        if (empty($errormsg)) {
            $errormsg = get_string("invalidcode", 'auth_int_keygen');
            $errorcode = 3;
        }
    }
}
...
if (empty($SESSION->wantsurl)) {
    $SESSION->wantsurl = (array_key_exists('HTTP_REFERER',$_SERVER) &&
                          ...
                          $_SERVER["HTTP_REFERER"] != $CFG->wwwroot.'/' &&
                          $_SERVER["HTTP_REFERER"] != $CFG->httpswwwroot.'/auth/int_keygen/' &&
                          strpos($_SERVER["HTTP_REFERER"], $CFG->httpswwwroot.'/auth/int_keygen/?') !== 0 &&
                          strpos($_SERVER["HTTP_REFERER"], $CFG->httpswwwroot.'/auth/int_keygen/index.php') !== 0) // There might be some extra params such as ?lang=.
...
if (isloggedin() and !isguestuser()) {
    ...
    $continue = new single_button(new moodle_url($CFG->httpswwwroot.'/auth/int_keygen/index.php', array('cancel'=>1)), get_string('cancel'), 'get');
